    ...
    if(window.localStorage.getItem("items")){
      numItems = parseInt(window.localStorage.getItem("items"));
    }else{
      window.localStorage.setItem("items", 0);
    }

    for (i = 0; i < numItems; i++) { 
        addItemTodo(window.localStorage.getItem("item"+numItems.toString()), false);
    }
	...
	function removeItem() {
        ...
        window.localStorage.setItem("items", window.localStorage.getItem("items") - 1);
	}
	...
	function addItemTodo(text, load) {
        ...
        if(load == true){
          numItems += 1;
          window.localStorage.setItem("items", numItems);
          window.localStorage.setItem("item"+String(numItems),text);
        }
	}
